...
static void *worker_thread(void *arg) {
    ...
    while (1) {
        ...
        if (!pool->running) {
            ...
            pthread_barrier_wait(&pool->barrier_end);
            break;
        }
...
int workers_init(WorkerPool *pool, EngineContext *ctx,
    ...
    pthread_barrier_init(&pool->barrier_start, NULL,
                         (unsigned)(thread_count + 1));
    pthread_barrier_init(&pool->barrier_end,   NULL,
                         (unsigned)(thread_count + 1));
...
void workers_destroy(WorkerPool *pool) {
    ...
    pthread_barrier_destroy(&pool->barrier_end);
    memset(pool, 0, sizeof(*pool));
}
