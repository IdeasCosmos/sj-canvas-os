...
int mve_tick(MultiverseEngine *mve, EngineContext *ctx) {
    ...
    for (uint32_t i = 0; i < LANE_ID_MAX; i++) {
        if (!(mve->lanes.active_mask[i/32] & (1u << (i%32)))) continue;

        /* Phase 5 TODO:
         *   1. Y-time 모드면 branch의 y 범위를 tick 기반으로 계산
         *   2. lane_tick() 호출 (lane_id 필터 적용)
         *   3. 해당 lane의 WH zone에 이벤트 기록
         *   4. BH zone에서 에너지 감소 처리
         */
        total += lane_tick(ctx, &mve->lanes, (uint8_t)i);
    }
